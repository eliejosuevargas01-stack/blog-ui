...
type TopicParams = {
  topicSlug?: string;
  slug?: string; // in case slug is used in dynamic catch-all route
};
...
export default function Topic({ lang, topicSlug: propsTopicSlug, initialPosts: propsInitialPosts }: TopicProps) {
  const t = translations[lang];
  const homePath = buildPath(lang, "home");
  const params = useParams<TopicParams>();
  const topicSlug = propsTopicSlug ?? params.slug ?? params.topicSlug;
