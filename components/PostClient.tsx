...
export default function Post({ lang, initialPosts: propsInitialPosts }: PostProps) {
  ...
  const [clientParsedContent, setClientParsedContent] = useState<string | null>(null);

  useEffect(() => {
    let isMounted = true;
    const needsParsing = (post?.content && !post.contentHtml && !hasInlineHtml) || (post?.contentHtml && !contentHtmlHasTags);
    if (!needsParsing) {
      return;
    }
  ...
  }, [post, hasInlineHtml, contentHtmlHasTags]);
  ...
  // IMPORTANTE: Busca a imagem do post do banco (post.img) ou das propriedades de fallback
  const coverImage = post?.img ?? post?.image ?? post?.images?.[0] ?? null;
  ...
  const markdownContent = clientParsedContent;
  ...
  const parsedBlocks: any[] = useMemo(() => {
    if (!post) return [];
            ...
            {post && !showLoading && !showError && (
              <article className="mt-10">
                <div className="max-w-5xl mx-auto">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {post.tags.map((tag) => {
                          ...
                          return (
                            <Link
                              key={tag}
                              href={`${articlesPath}?tag=${encodeURIComponent(normalizedTag)}`}
                  ...
                  {/* RENDERIZAÇÃO DO CONTEÚDO DO ARTIGO (POR BLOCOS OU HTML) */}
                  <div className="mt-10">
                    {parsedBlocks.length > 0 ? (
                      <div className="space-y-6">
                        {parsedBlocks.map((block: any, i: number) => {
                          ...
                          return (
                            <div key={i} className="flex flex-col gap-6">
                              ...
                              {block.image && !hasImageInText && !isImageAlreadyInText && (
                                <div className="overflow-hidden rounded-2xl border border-border/80 bg-muted/50 my-2">
                                  <div className="aspect-video w-full relative">
                                    <Image
                                      src={block.image}
                                      alt={`Ilustração ${i + 1}`}
                        ...
                        <div className="rounded-2xl border border-border bg-card/70 p-6">
                          ...
                          >
                            <div className="relative aspect-[16/9] w-full overflow-hidden bg-muted">
                              {guidePost.imageThumb || guidePost.image || guidePost.img ? (
                                <Image
                                  src={guidePost.imageThumb ?? guidePost.image ?? guidePost.img ?? ""}
                          ...
                          <div className="grid gap-4 sm:grid-cols-2">
                            {relatedPosts.map((related) => {
                              ...
                              const relatedPath = relatedSlug
                                ...
                                : buildPath(lang, "home");
                              const relatedImage =
                                related.imageThumb ?? related.image ?? related.img ?? null;
                              return (
                                <Link
                                  key={related.id}
                                  href={relatedPath}
